...
Usually all you need to ruun is `docker compose restart` - but if that fails because the app is not running you can use `docker compose up -d`. Replace `docker` with `podman` if being asked or previously told to use podman instead of docker.
